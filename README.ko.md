...
이미 운영 중인 self-managed Slurm/ParallelCluster cluster 대신 `vla-ft`를 쓰는 구체적
이유가 바로 이것입니다: **requeue/resume launcher를 손으로 짜지 않습니다.** Spot은 기본
on이고 회수는 자동 복구되지만, *메커니즘은 backend마다 다르고 성숙도도 다릅니다* — 정직하게
적습니다:

- **Pattern A (Batch)** — 컨테이너가 **EFS**에 checkpoint를 저장하고, Spot 회수 시 Batch가
  job을 retry하면 launcher가 step 0이 아니라 EFS의 최신 checkpoint에서 재개합니다. 이
  경로는 **wired돼 있고 실제 run이 SUCCEEDED**; 다만 mid-run 회수→재개 *전환 자체*를 따로
  강제하진 않았으므로(검증 run은 회수 전에 완료) resume 경로는 wired이지 reclaim-proven은
  아닙니다.
- **Pattern B (SageMaker)** — SageMaker **Managed Spot Training**을 사용하며, checkpoint
  업로드/복원이 서비스에 native입니다. `vla-ft` 내에서는 **코드 완성, 아직 배포 검증 안
  됨**입니다.

어느 쪽이든 resume wiring은 플랫폼이 소유합니다. self-managed Slurm/ParallelCluster
cluster에서는 Spot interruption 처리·requeue·checkpoint 복원이 **직접 짜고 유지해야 하는
몫**입니다 — 이 DIY launcher가 정확히 제거되는 대상입니다. (checkpoint 저장 자체는 어떤
backend에서든 항상 학습 스크립트의 책임입니다 — S3/EFS/FSx 저장; `vla-ft`가 더하는 것은 그
주위의 *회수 시 재개 wiring*입니다.)
...
├── mcp/                       선택적 MCP server (구현됨, 7개 툴) — agent session에서 submit / monitor / checkpoint 검증
...
묶지 않은 네 가지를 결합합니다. 아래 각 축은 **성숙도를 인라인으로** 함께 표기합니다 —
capability와 verified는 다르며, 컴포넌트별 전체 내역은 [상태](#상태) 섹션에 있습니다:

- **Ease** — 단일 명령, smart default, pre-flight 점검; 손으로 쓰는 launcher 없음.
  *(구현, dry-run 검증; live `--yes` submit이 남은 점검.)*
- **Efficiency** — Spot + AZ 선택 + 인스턴스 auto-sizing + early-stop을 기본값으로,
  실행 *전에* 비용 추정 제공. 그리고 Spot이 인스턴스를 회수하면 job이 처음부터 다시
  시작하는 게 아니라 **마지막 checkpoint에서 재개**합니다 — self-managed
  Slurm/ParallelCluster cluster라면 직접 소유해야 하는 requeue/resume launcher를 손으로
  짤 필요가 없습니다 (Pattern A는 이 경로가 wired돼 있고 실제 run이 SUCCEEDED;
  Pattern B의 managed-spot resume은 코드 완성, 아직 배포 검증 안 됨 —
  [Spot 회수: 재시작이 아닌 재개](#spot-회수-재시작이-아닌-재개) 참고).
- **Coverage** — IL *과* RL을 하나의 진입점 뒤에. *(IL Pattern A는 end-to-end 검증;
  RL은 코드 완성, 아직 GPU 실행 안 됨 — 구현됨, 전달 안 됨.)*
- **Backend 유연성** — 같은 intent를 Batch, SageMaker Training Job, HyperPod(A/B/C)에서
  실행, override 가능. *(A 검증; B 코드 완성, 배포 대기; C 코드 + `cdk synth`만.)*

[`docs/ARCHITECTURE.md` §6](docs/ARCHITECTURE.md)에 정직한 prior-art 비교가 있습니다 —
기존 AWS sample과 겹치는 지점과 실제로 차별화되는 지점을 모두 포함합니다. 이미
**self-managed ParallelCluster / Slurm** cluster를 운영(또는 멀티클라우드 운영)한다면,
[§6.1](docs/ARCHITECTURE.md)에 `vla-ft`가 더하는 것과 — 똑같이 정직하게 — 더하지 않는 것
(AWS 전용이며 ParallelCluster 대체가 아님)이 정리돼 있습니다.
...
- [`docs/MCP-DESIGN.md`](docs/MCP-DESIGN.md) — 선택적 MCP server 설계 근거.
  server는 **구현 + live 검증 완료**(v1, 7개 툴 — `submit_finetune`,
  `get_job_status`("정말 학습 중인가?"), `describe_checkpoint`(export consistency gate),
  `list/get_job`, `register/list_checkpoints`); as-built surface와 등록 방법은
  [`mcp/README.md`](mcp/README.md) 참고.
